...
class ChaCha20Encryption:
    ...
    def __init__(self):
        """初始化ChaCha20加密"""
        self.logger = Logger("ChaCha20Encryption")
    ...
    def encrypt(self, data: bytes, key: bytes = None) -> Tuple[bytes, Dict[str, Any]]:
        ...
        try:
            ...
            nonce = os.urandom(16)  # 128位nonce (cryptography库要求)
            ...
            self.logger.debug(f"ChaCha20加密完成，数据大小: {len(encrypted_data)}")
            return encrypted_data, metadata

        except ImportError:
            self.logger.error("cryptography库未安装，无法使用ChaCha20加密")
            # 回退到简单XOR加密
    ...
    def decrypt(self, encrypted_data: bytes, metadata: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            self.logger.debug(f"ChaCha20解密完成，数据大小: {len(decrypted_data)}")
