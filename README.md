...
`config/security_settings.json` 中声明了例如：

- **PBKDF2-SHA256** 密钥派生（迭代次数、盐长度可配；偏执档可指向 Argon2）
- 密钥混淆 / 分片 / 诱饵密钥数量
- 反逆向：字符串加密、控制流扁平化、反调试、VM 检测、时序检测等策略开关
- 运行时：安全删除、完整性检查、审计日志事件列表
- 性能：线程数、缓冲、分块等

运行时实现见 `src/security/anti_reverse.py`（调试器 / 远程调试 / `sys.gettrace` / 简单时序检测等）。

> 说明：配置文件描述的是**安全策略目标**；具体能力以当前源码实现为准，部分打包/加固项需结合 PyInstaller 等外部工具。
